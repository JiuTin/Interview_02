...
namespace MagicaCloth
{
    ...
    public class PhysicsManagerTeamData : PhysicsManagerAccess
    {
        ...
        public struct TeamData
        {
            ...
            /// <summary>
            /// 自身のボーンのクロスデータ作成時のスケール長
            /// </summary>
            public float baseBoneScale;
        ...
        public void SetBoneIndex(int teamId, int boneIndex, Vector3 baseBoneScale)
        {
            TeamData data = teamDataList[teamId];
            data.boneIndex = boneIndex;
            data.baseBoneScale = baseBoneScale.magnitude; // ベクトル長
            teamDataList[teamId] = data;
        }
        ...
        struct PreProcessTeamDataJob : IJobParallelFor
        {
            ...
            public void Execute(int teamId)
            {
                ...
                else
                {
                    ...
                    // チームスケール算出
                    if (tdata.baseBoneScale > 0.0f)
                    {
                        tdata.teamScale = math.length(bscl) / tdata.baseBoneScale;
                    }
                    ...
                    if (wdata.resetTeleport == 1)
                    {
                        ...
                        if (moveLen >= wdata.teleportDistance * tdata.teamScale || math.degrees(MathUtility.Angle(moveRot)) >= wdata.teleportRotation)
                        {
